...
const GameChoice = ({gameType, heading, tagLine, onChoice}) =>{
    return (
        <div className={`choice-container-${gameType}`}>
            ...
            <ChoiceButton type="choice" choice={`start-game-${gameType}`} label="Start Game" onChoice={onChoice}/>
            <ChoiceButton type="choice" choice={`join-game-${gameType}`} label="Join Game" onChoice={onChoice}/>
        </div>
    )   
}
